 ...
 * - [32]  - packet HMAC
 *
 * Each packet contains a variable length payload where the
 * first 65 bytes are used by the current node to construct a forwarding
 * packet containing the remaining bytes of the payload.
 * - [33]  - public key of node where we should forward the payload
 * - [32]  - HMAC covering the next packet constuction
 * - [var] - unencrypted data that should be forwarded to the next node
 *
 * @param packet
 * @param nodeKey
 */
